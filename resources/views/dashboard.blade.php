...
@section('header')
  <div class="bg-white shadow border-b border-gray-200">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <h1 class="text-4xl font-extrabold text-gray-900">Hello, {{ Auth::user()->name }}!</h1>
      <p class="mt-2 text-lg text-gray-700">Welcome back—here’s the latest on your spending.</p>
    </div>
  </div>
@endsection
        ...
        <form method="GET" action="{{ route('reports.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            ...
            <input
              ...
              name="from"
              value="{{ $from }}"
              class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-sky-500 focus:border-sky-500"
            />
          </div>
          <div>
          ...
          <div class="flex items-end">
            <button
              type="submit"
              class="w-full bg-sky-500 text-white font-medium py-3 rounded-lg hover:bg-sky-600 transition"
            >
              Apply Filter
            </button>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="flex flex-col space-y-4">
          <a
            ...
            class="block bg-sky-500 text-white font-medium py-3 rounded-lg text-center hover:bg-sky-600 transition"
          >
            + Add Expense
          </a>
          <a
            href="{{ route('reports.pdf',   ['from' => $from, 'to' => $to]) }}"
            class="block bg-rose-500 text-white font-medium py-3 rounded-lg text-center hover:bg-rose-600 transition"
          >
            Export PDF
          </a>
          <a
            href="{{ route('reports.excel', ['from' => $from, 'to' => $to]) }}"
            class="block bg-emerald-500 text-white font-medium py-3 rounded-lg text-center hover:bg-emerald-600 transition"
          >
            Export Excel
          </a>
        </div>
